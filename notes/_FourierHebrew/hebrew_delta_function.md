...
מקרה ראשון בו אנחנו ממש יכולים לחשב את האינטגרל הזה הוא כאשר נסתכל על הפונקצית מדרגה $f(x)=\chi_{[a,b]}(x)$ ואז נקבל את האינטגרל
...
$$,f_1(x)=f(x)-\frac{f(0^+)+f(0^-)}{2}\chi_{[-1,1]}(x)$$
והפונקציה החדשה $f_1(x)$ תקיים ש $f_1(0^+)=-f_1(0^-)$ ובנוסף
 $$.\int_{-\infty}^\infty f(x)\frac{\sin(Mx)}{\pi x} \dx = \int_{-\infty}^\infty f_1(x)\frac{\sin(Mx)}{\pi x} \dx + \frac{f(0^+)+f(0^-)}{2}$$
 במילים אחרות, מספיק שנבין את האינטגרל לפונקציות מהצורה $f_1$ שהגבולות החד צדדיים שלהן בראשית הם "אי זוגיים".
 
## פונקציות אי זוגיות
אם נרצה לעבור לפונקציה כללית יותר שעדיין קל לחשב את האינטגרל, אז מקום מאוד פשוט להתחיל בו זה עם פונקציות אי זוגיות, למשל
...
כמובן, שעבור פונקציות אי זוגיות, הממוצע של הגבולות בראשית (בהנחה והם קיימים) גם חייב להיות אפס, כלומר שוב קיבלנו את 
$$.\frac{f(0^+)+f(0^-)}{2}$$

בשלב הזה, נוכל לחשב את האינטגרל לכל פונקציה $f$ שהיא קומבינציה לינארית של פונקציות מדרגה ושל פונקציות אי זוגיות. כמה "חסר" לנו כדי להבין פונקציה כללית? אם $f\in E^1(\RR)$ פונקציה כלשהי שיש לה גבולות חד צדדיים בראשית, אך נוכל "לתקן אותה" בצורה הבאה:
1. נגדיר את הפונקציה 
   $$,f_1(x)=f(x)-\frac{f(0^+)+f(0^-)}{2}$$
   שזו עדיין פונקציה אינטגרבילית בהחלט, שעבורה מתקיים ש $\frac{f_1(0^+)+f_1(0^-)}{2}=0$, כלומר תיקנו את הפונקציה כך שממוצע הגבולות החד צדדיים יהיה אפס.
2. נגדיר את הפונקציה 
