...
אז הפונקציה $f(x)g(x)$ היא גם פולינום:
$$,f(x)g(x) = \sum_0^{2d} c_kx^k$$
ולכן נוכל לשאול איך מבטאים את המקדמים שלה $c_k$ ע"י המקדמים $a_k,b_k$ של $f(x),g(x)$. במקרה הזה נקבל ש :
...
לכן, סה"כ קיבלנו את ההתאמה:
$$\align{f(x) & \Rightarrow (a_0, a_1, ...)\\
g(x) & \Rightarrow (b_0, b_1, ...) \\
f(x)\cdot g(x) & \Rightarrow (c_0,c_1,...),\; c_k=\sum_{i=0}^k a_i b_{k-i}}$$
