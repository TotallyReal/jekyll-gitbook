...
Having so many "real functions" types, and having them representing so many objects in our world, it is very important to find mathematical structures and tools to study them. We already learned some of them in basic courses in calculus and algebra:
...
This separation idea is actually quite simple, and we have been using it already just in a different setting. Indeed, if we think about the information stream as a point in the plane $\RR^2$, then the different channel correspond to the different coordinates. For example, one of the "channels" of $(x,y)$ is simply $x$. In this simple case we know how to take different "channels" $x$ and $y$ and combine them into one stream $(x,y)$, but also how to separate them back. One of the main goal for this course is to do the same with our functions $f$ from above which live in an infinite dimensional space.

## Image compression

In our modern day life, we can images and videos of almost everything that happens to us. More over, as our technology improves, we can save more and more information in each image. However, the problem with this approach is that this information takes a lot of space. One solution is to keep buying new space, whether it is a new hard drive or extra cloud space. Another solution is to try compressing the information instead. There are many compression algorithms and one of them (which for example is used in the JPEG image compression) is using Fourier transformation.
...
Suppose we have a grayscale image file, where the color of each pixel is represented by an integer in $[0,255]$ going from black at $0$ all the way to white at $255$. One very simple way to compress the image, is to change the pixel value from 255 options to something smaller, for example 16 or 8. This already gives us some compressions, and for some images it will be hard to notice the changes.
 ![compression](../../images/motivation/compression_pixelwise.png)
...
This idea of looking for patterns is measured in a sense in the Fourier coefficient. The Fourier transform decompose the full image (and not each pixel separately) into sine and cosine waves with certain weights which determines how "strong" that pattern is in the picture. As in turns out, most weights are quite small, and removing their corresponding patterns completely doesn't change the image too much. Thus, we can throw away all the information about this small weight waves, and get a compression without losing too many details:

 ![compression](../../images/motivation/compression_fourier.png)
See some actual python code which runs the compressions above: [Python image compression](https://colab.research.google.com/drive/1y2M0qtu4bkMG5eILY_LVRvEEK61UD3OS?usp=sharing).
...
To summarize, one way (of many) to think about Fourier transform is as extracting patterns from our data and the weight for each pattern. If the data has nice "wavy" data, then the Fourier transform can help understanding it.

---
# The tools of our crafts
...
You first learned about this idea back in your first course in linear algebra, which had the mysterious name of eigenvalues and eigenvectors. This was further expanded in the second course, about inner product spaces, where you saw the importance of geometry and working with orthonormal basis of eigenvalues.
...
## Sines, cosines and the complex plane
As mention several times, the sine and cosine functions are going to be our building blocks for the Fourier transform. At first glance, it is not clear why it happens and why they are so important, and in particular each one of one them on its own. However, they become quite interesting once combined - they are just two parts ($x$ and $y$ coordinates) of the same very simple and very important process: Moving at constant speed on a circle. This clues us why these trigonometric functions arise so naturally - a lot of natural phenomena have natural "rotational" behavior, and we see the just the sine or cosines function, just because we don't see the "whole picture".

<center><img src="../../images/motivation/pendulum.png" height="200"  /><img src="../../images/motivation/solar system.jpg" height="200"  /> </center>
